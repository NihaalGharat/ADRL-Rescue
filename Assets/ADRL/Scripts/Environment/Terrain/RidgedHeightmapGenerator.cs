namespace ADRL.Environment.Terrain
{
    ...
    public class RidgedHeightmapGenerator : IHeightmapGenerator
    {
            ...
            for (var z = 0; z < resolution; z++)
            {
                for (var x = 0; x < resolution; x++)
                {
                    ...
                    for (var o = 0; o < octaves; o++)
                    {
                        ...
                        var signal = Mathf.PerlinNoise(nx, nz);
                        signal = (signal - NoiseCenter) * NoiseScaleFactor;
                        signal = 1f - Mathf.Abs(signal);
                        signal *= signal;
                        noiseValue += signal * amp;
                        freq *= lacunarity;
                        amp *= persistence;
                    }
